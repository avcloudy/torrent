...
public func decode(data: String) throws -> bencode? {
  guard let asciiData = data.data(using: .ascii) else { throw bencodeError.nonASCII }
...
private func dechunk(stream: [Character], index: Int) throws -> (bencode?, Int) {
  ...
  guard index < stream.count else { throw bencodeError.streamEmpty }
  let nextCharacter = stream[index]
  switch nextCharacter {
  case inNumberRange: return try decodeString(stream: stream, index: index)
  case "i": return try decodeInt(stream: stream, index: index)
  case "l": return try decodeList(stream: stream, index: index)
  case "d": return try decodeDict(stream: stream, index: index)
...
private func decodeString(stream: [Character], index: Int) throws -> (bencode?, Int) {
  ...
  guard stream[index] == ":" else { throw bencodeError.malformedString(index: index) }
  index += 1
...
private func decodeInt(stream: [Character], index: Int) throws -> (bencode?, Int) {
  ...
  guard stream[index] == "i" else { throw bencodeError.malformedInt(index: index) }
  index += 1
  var intChars = ""
  while stream[index].isNumber {
  ...
  guard stream[index] == "e" else { throw bencodeError.intMissingEnd(index: index) }
  index += 1
...
private func decodeList(stream: [Character], index: Int) throws -> (bencode?, Int) {
  ...
  guard stream[index] == "l" else { throw bencodeError.malformedList(index: index) }
  index += 1
  // check next character is not e, if it is, return an empty bencode list
  guard stream[index] != "e" else { return (.list([]), index) }
  var list: [bencode] = []
  while stream[index] != "e" {
    let (bencodeObject, newIndex) = try dechunk(stream: stream, index: index)
    index = newIndex
    if let value = bencodeObject { list.append(value) }
  }
  index += 1
...
private func decodeDict(stream: [Character], index: Int) throws -> (bencode?, Int) {
  ...
  guard stream[index] != "e" else { return (.dict(dict), index) }
  while stream[index] != "e" {
    ...
    guard newIndex < stream.count else { throw bencodeError.streamEmpty }
    index = newIndex
    let (value, secondNewIndex) = try dechunk(stream: stream, index: index)
    dict[key ?? .string("")] = value ?? .string("")
    if let keyObject = key { dict[keyObject] = value }
    guard secondNewIndex < stream.count else { throw bencodeError.streamEmpty }
...
public func walker(bencodedObject: bencode) throws -> Any {
  ...
  case .dict(let d):
    var dict: [String: Any] = [:]
    for (key, val) in d {
      guard case .string(let keyString) = key else { throw bencodeError.dictKeyNotStringWalker }
